...
interface ClassInfo { id: number; classId: number; className: string; classType: string }
interface DrugModel {
  id: number; name: string; ndc: string; form: string; strength: string; classId: number; classType: string; className: string;
  acq: number; awp: number; rxcui: number; route: string; teCode: string; ingrdient: string; applicationNumber: string; applicationType: string; strengthUnit: string; type: string;
}
interface Prescription { net?: number; ndc?: string; drugName?: string; drugClassId?: number; insuranceId?: number }
...
const InsuranceSearch2: React.FC = () => {
  ...
  const classType = (typeof window !== "undefined" && localStorage.getItem("classType")) || "ClassV1";
  ...
  const debouncedBinSearch = useCallback(
    debounce(async (text: string) => {
      ...
      if (!text.trim()) { setBinSuggestions([]); setShowBinSuggestions(false); return; }
      try {
        setApiError(null);
        const { data } = await publicApiRef.current.get<BinModel>(`/drug/GetInsurancesBinsByName?bin=${encodeURIComponent(text)}`);
  ...
  useEffect(() => { if (!ready) return; debouncedBinSearch(binQuery); }, [binQuery, ready, debouncedBinSearch]);
  ...
  const onSelectBin = async (bin: BinModel) => {
    ...
    // reset downstream
    setPcnList([]); setSelectedPcn(null); setRxGroups([]); setSelectedRxGroup(null);
  ...
  const onSelectRxGroup = (rg: RxGroupModel) => {
    setSelectedRxGroup(rg); setRxGroupSearchQuery(rg.rxGroup); setShowRxGroupSuggestions(false);
    setDrugSearchQuery(""); setClassInfos([]); setPage(1); setDrugs([]); setSelectedDrug(null); setNdcList([]); setSelectedNdc(""); setNetDetails(null);
  };
  ...
  const fetchClasses = useCallback(
    async (pageNumber: number, append = false) => {
      if (!publicApiRef.current) return;
      const q = drugSearchQuery.trim(); if (!q) { setClassInfos([]); setPage(1); return; }

      let url = "";
      if (limitSearch) {
        if (selectedRxGroup) {
          url = `/drug/GetDrugClassesByInsuranceNamePagintated?insurance=${encodeURIComponent(selectedRxGroup.rxGroup)}&drugClassName=${encodeURIComponent(q)}&pageNumber=${pageNumber}&pageSize=${PAGE_SIZE}&classVersion=${encodeURIComponent(classType)}`;
        } else if (selectedPcn) {
          url = `/drug/GetDrugClassesByPCNPagintated?insurance=${encodeURIComponent(selectedPcn.pcn)}&drugClassName=${encodeURIComponent(q)}&pageNumber=${pageNumber}&pageSize=${PAGE_SIZE}&classVersion=${encodeURIComponent(classType)}`;
        } else if (selectedBin) {
          url = `/drug/GetDrugClassesByBINPagintated?insurance=${encodeURIComponent(selectedBin.bin)}&drugClassName=${encodeURIComponent(q)}&pageNumber=${pageNumber}&pageSize=${PAGE_SIZE}&classVersion=${encodeURIComponent(classType)}`;
        } else { setClassInfos([]); return; }
      } else {
        url = `/drug/GetClassesByName?name=${encodeURIComponent(q)}&pageNumber=${pageNumber}&pageSize=${PAGE_SIZE}&classVersion=${encodeURIComponent(classType)}`;
      }
  ...
  useEffect(() => {
    const el = classListRef.current; if (!el) return;
  ...
  const onSelectClass = async (ci: ClassInfo) => {
    if (!publicApiRef.current) return;
    setDrugs([]); setSelectedDrug(null); setNdcList([]); setSelectedNdc(""); setNetDetails(null); setShowDrugSuggestions(false);
    try {
      setApiError(null);
      const { data } = await publicApiRef.current.get<DrugModel[]>(`/drug/GetDrugsByClassId?classId=${encodeURIComponent(ci.classId)}&classType=${encodeURIComponent(ci.classType)}`);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setApiError(null);
        const { data } = await publicApiRef.current.get<Prescription>(`/drug/GetDetails?ndc=${encodeURIComponent(selectedNdc)}&insuranceId=${encodeURIComponent(String(selectedRxGroup.id))}`);
        setNetDetails(data ?? null);
      } catch (e: any) {
        if (e?.response?.status === 401 || e?.response?.status === 403) { setNetDetails(null); setApiError("GetDetails returned 401/403 (public access blocked)."); }
  ...
  const clearAll = () => {
    ...
    setDrugSearchQuery(""); setClassInfos([]); setShowDrugSuggestions(false); setPage(1);
    setDrugs([]); setSelectedDrug(null); setNdcList([]); setSelectedNdc(""); setNetDetails(null);
    setApiError(null);
  };
