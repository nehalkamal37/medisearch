// src/pages/new/UserActivityLogs.tsx
...
const normalizeToActionEnum = (key: string): Action | string => {
  ...
  if (k.includes("search") && k.includes("ndc")) return "SEARCH_DRUG";
...
const StatCard: React.FC<{
  ...
  footNote?: string;
  colClass?: string;   // ← NEW
}> = ({ title, value, iconClass, gradientFrom, gradientTo, footNote, colClass }) => (
  <div className={colClass ?? "col-12 col-sm-6"}>
    <div
    ...
    >
      <div className="card-body py-3">
        ...
        <h3 className="mt-2 mb-1 fw-bolder">
          {Number.isFinite(+(value as any)) ? (+value).toLocaleString() : value}
        </h3>
      ...
      <div
        ...
        style={{
          width: 160, height: 160, right: -40, bottom: -40,
...
const UserActivityLogs: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchLogs = async () => {
      try {
        const res = await axiosInstance.get<ApiLog[]>(`/Logs/GetLogs`, {
          ...
          .filter((l) => {
            const key =
              typeof l.action === "string"
                ...
                : (l.action as string);
            return (
              Object.keys(actionNameMap).includes(key) ||
              key.toLowerCase().startsWith("viewdrugdetails")
            );
          });
  ...
  const userSuggestions = useMemo(() => {
    ...
    return unique.filter((u) =>
      u.toLowerCase().includes(filters.user.toLowerCase())
    );
  }, [logs, filters.user]);
  ...
  const actionSuggestions = useMemo(() => {
    const unique = Array.from(
      new Set(
        logs.map((l) => (typeof l.action === "string" ? l.action : l.action))
      )
    );
    return unique.filter((a) =>
      a.toLowerCase().includes((filters.action || "").toString().toLowerCase())
    );
  }, [logs, filters.action]);
  ...
  useEffect(() => {
    ...
    if (filters.action) {
      result = result.filter(
        (log) =>
          (typeof log.action === "string" ? log.action : log.action) ===
          filters.action
      );
    }
  ...
  const clearFilters = () =>
    setFilters({ user: "", action: "", dateFrom: "", dateTo: "", search: "" });
  ...
  const lineOptions = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: filters.user ? `Activity for ${filters.user}` : "Logs Per Day",
      },
    },
  };
  ...
  const actionChartData = useMemo(() => {
    const counts: Record<string, number> = {};
    filteredLogs.forEach((l) => {
      const key = (typeof l.action === "string" ? l.action : l.action).trim();
  ...
  const barOptions = {
    ...
    plugins: {
      legend: { position: "top" as const },
      title: { display: true, text: "Action Usage Overview (Last Word)" },
    },
  };
  ...
  const workingHoursData = useMemo(() => {
    ...
    const userLogs = logs.filter(
      (l) => l.user.toLowerCase() === filters.user.toLowerCase()
    );
    ...
    Object.keys(grouped).forEach((day) => {
      ...
      const loginLike = arr.filter(
        (l) =>
          (typeof l.action === "string" ? l.action.toString() : l.action) ===
            "LOGIN" || (l.rawAction || "").toLowerCase() === "login"
      );
      const first = (loginLike.length > 0 ? loginLike : arr).reduce((a, b) =>
  ...
  const workingHoursChartData = useMemo(() => {
    ...
    return {
      labels,
      datasets: [
        { label: "Normal Hours", data: normal, backgroundColor: "rgba(54,162,235,0.6)" },
        { label: "Extra Hours", data: extra, backgroundColor: "rgba(255,99,132,0.6)" },
      ],
    };
  }, [workingHoursData]);
  ...
  const workingHoursChartOptions = {
    ...
    plugins: {
      legend: { position: "top" as const },
      title: { display: true, text: "Working Hours Breakdown per Day" },
    },
  ...
  return (
    <div className="content-area py-3 logs-page">
            ...
            <div className="card shadow-lg border-0 rounded-4 overflow-hidden">
              <div className="card-header py-4 px-5">
                <h4 className="mb-0 fw-semibold">
                  <i className="ti ti-history me-2"></i>User Activity Logs
                </h4>
                    ...
                    <div className="card bg-light border-0">
                      <div className="card-body">
                        <h6 className="card-title mb-3">
                          <i className="ti ti-filter me-2"></i>Filter Logs
                        </h6>
                          ...
                          <div className="col-md-3" ref={userRef}>
                            ...
                            <div className="position-relative">
                              ...
                              {filters.user && (
                                <button
                                  type="button"
                                  className="btn btn-sm btn-link position-absolute end-0 top-0 mt-2 me-2 text-muted"
                                  onClick={() => setFilters((p) => ({ ...p, user: "" }))}
                                >
                                  &times;
                                </button>
                              )}
...
{/* Search + Clear (button on the right) */}
<div className="row mt-3 align-items-center">
  ...
  <div className="col-12 col-md-auto mt-2 mt-md-0 d-flex justify-content-md-end">
    <button
...
<div className="row g-2 mb-2 justify-content-between">
  ...
  <StatCard
    ...
    gradientFrom="#059669"
    gradientTo="#22C55E"
    colClass="col-12 col-md-5 ms-md-auto"   // right
  />
</div>


                {/* Charts (visible once a user is chosen) */}
                {filters.user ? (
                  ...
                  <div className="text-center text-muted mb-4">
                    Select a user to view activity charts and working hours.
                  </div>
                )}
                      ...
                      {paginatedLogs.length > 0 ? (
                        paginatedLogs.map((log) => (
                          <tr key={log.id} className="align-middle">
                            ...
                            <td>
                              <span className="d-block text-truncate" style={{ maxWidth: 180 }}>
                                {log.user}
                              </span>
                            </td>
                            ...
                            <td className="text-truncate" style={{ maxWidth: 260 }} title={log.details}>
                              {log.details}
                            </td>
                          </tr>
                        ))
                ...
                {/* Pagination */}
                {/* ... pagination + export buttons unchanged ... */}
                {/* (keep your existing pagination + export code here) */}
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Centering styles for this page only */}
      <style>{`
        .content-area{ padding-left: 1rem; padding-right: 1rem; }
        .gw-wide{ max-width: clamp(1100px, 78vw, 1320px); margin-inline: auto; }
        .logs-page{ margin-left: 0 !important; } /* neutralize any sidebar push */
        @media (max-width: 991.98px){ .gw-wide{ max-width: 100%; } }
      `}</style>
    </div>
  );
};
