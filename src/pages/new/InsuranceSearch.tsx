...
const InsuranceSearch: React.FC = () => {
  ...
  /** Prevent dropdown reopening right after picking a drug */
  const suppressNextDrugSearch = useRef(false);
  ...
  const debouncedDrugSearch = useCallback(
    debounce(() => {
      if (suppressNextDrugSearch.current) {
        suppressNextDrugSearch.current = false;
        setShowDrugSuggestions(false);
        return;
      }
      if (!drugSearchQuery.trim()) {
  ...
  const onPickDrug = (drug: DrugModel) => {
    ...
    suppressNextDrugSearch.current = true;
  ...
  return (
    <div className="content-area py-3 insurance-page">
  ...
  <div className="d-flex flex-column align-items-center text-center mb-3 fs-5">
    <AutoBreadcrumb title="Insurance Search" />
  </div>

      
      {/* Wider lane like DrugSearch */}
  <div className="container-xxl gw-wide">
    <div className="row justify-content-center">
      <div className="col-12 col-lg-11 col-xl-8">
  ...
  <style>{`
    ...
    /* center breadcrumb on this page */
    .insurance-page .breadcrumb{ justify-content:center !important; }
    .insurance-page .breadcrumb .breadcrumb-item{ white-space:nowrap; }

    /* neutralize the old layout rule if it remains in the bundle */
