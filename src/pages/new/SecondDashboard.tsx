// src/pages/Dashboard.tsx  (SecondDashboard - matching scripts + alias-aware fetch)
        ...
        <div className="card-body">
          <div className="d-flex justify-content-between align-items-start">
            <div className="pe-3">
              ...
              <h3 className="fw-bold mb-0">{prefix}{formatNumber(val, decimals)}</h3>
            </div>
...
const SecondDashboard: React.FC<Props> = ({ data }) => {
  ...
  // Sync RxGroup selection with localStorage (this page controls Rx only)
  const [selectedInsurance, setSelectedInsurance] = useState(
    (localStorage.getItem("selectedRx") || "").trim()
  );

  // Read possible values set by other pages (PCN/BIN)
  const selectedPcn = (localStorage.getItem("selectedPcn") || "").trim();
  const selectedBin = (localStorage.getItem("selectedBin") || "").trim();
  ...
  useEffect(() => {
    ...
    const fetchWithAliases = async () => {
      try {
        ...
        const baseParams: Record<string, any> = { classVersion, matchOn };

        // Choose which value to send based on matchOn
        const rxVal  = (selectedInsurance || (localStorage.getItem("selectedRx") || "")).trim();
        const pcnVal = selectedPcn;
        const binVal = selectedBin;

        // Build alias attempts depending on matchOn + value presence
        const aliasAttempts: Array<Record<string, any>> = [];

        if (matchOn === "RX" && rxVal) {
          aliasAttempts.push({ rx: rxVal });
          aliasAttempts.push({ rxGroup: rxVal });
          aliasAttempts.push({ rxgroup: rxVal });
          aliasAttempts.push({ rxGroupId: rxVal });
          aliasAttempts.push({ rxgroupid: rxVal });
          aliasAttempts.push({ rxGroupID: rxVal });
        } else if (matchOn === "PCN" && pcnVal) {
          aliasAttempts.push({ pcn: pcnVal });
          aliasAttempts.push({ pcnName: pcnVal });
          aliasAttempts.push({ pcnCode: pcnVal });
          aliasAttempts.push({ pcnId: pcnVal });
        } else if (matchOn === "BIN" && binVal) {
          aliasAttempts.push({ bin: binVal });
          aliasAttempts.push({ binCode: binVal });
          aliasAttempts.push({ binId: binVal });
        } else {
          // No server-side filter if we have no value for current matchOn
          aliasAttempts.push({});
        }
        ...
        for (const alias of aliasAttempts) {
          page = 1; all = [];
          try {
            // paginate until shorter page
            while (true) {
              const params = { ...baseParams, ...alias, pageNumber: page, pageSize };
        ...
        if (!success) {
          const details = lastErr?.response?.data?.message || lastErr?.message || "Unknown error";
          throw new Error(`Failed to load data. ${details}`);
        }

        if (!cancelled) setLoading(false);
      } catch (e: any) {
        if (!cancelled) {
          const msg = e?.message || e?.response?.data?.message || "Failed to load data.";
          setError(msg);
          setLoading(false);
        }
      }
  ...
  }, [matchOn, classVersion, data, selectedInsurance, selectedPcn, selectedBin]);

  // pick the live array (from props or server)
  const baseRows = useMemo<DrugTransaction[]>(
    () => (data && data.length ? data : serverData),
    [data, serverData]
  );

  // ===== ONLY MATCHING SCRIPTS (ndcCode === highestDrugNDC) =====
  const matchedRows = useMemo<DrugTransaction[]>(
    () => (baseRows || []).filter(i => (i?.ndcCode || "") === (i?.highestDrugNDC || "")),
    [baseRows]
  );

  // ===== state for filters / table =====
  const [latestScripts, setLatestScripts] = useState<DrugTransaction[]>(matchedRows);
  const [filteredData, setFilteredData] = useState<DrugTransaction[]>([]);
  const [currentPage, setCurrentPage] = useState(1);

  // NOTE: sortConfig.key is string so we can sort by derived keys like "difference"
  ...
  // KPIs (matching set)
  const [belowNetPriceCount, setBelowNetPriceCount] = useState(0); // count where netProfit < highestNet
  const [totalRevenue, setTotalRevenue] = useState(0);             // sum(netProfit)
  const [totalDeviation, setTotalDeviation] = useState<number>(0); // sum(highestNet - netProfit)
  ...
  useEffect(() => { setLatestScripts(matchedRows); }, [matchedRows]);
  ...
  // filter + sort + KPIs (for matched rows)
  useEffect(() => {
  ...
  const resetFilters = () => {
    ...
    localStorage.setItem("selectedRx", ""); // keep localStorage in sync
  };
 ...
 {/* ===== matchOn like MainDashboard ===== */}
  ...
  >
    ...
    <option value="RX">RxGroup</option>
          ...
          <h3 className="text-4xl fw-bolder text-primary mb-4 text-center">Estimated Best Net Differences</h3>
            ...
            <>
              <div className="d-flex align-items-center justify-content-between mb-2">
                ...
                <div className="btn-group">
                  <button type="button" className={`btn btn-sm btn-outline-light ${kpiLayout === "row" ? "active" : ""}`} onClick={() => { setKpiLayout("row"); localStorage.setItem("kpiLayout","row"); }}>
                    <i className="ti ti-layout-navbar" />
                  </button>
                  <button type="button" className={`btn btn-sm btn-outline-light ${kpiLayout === "grid" ? "active" : ""}`} onClick={() => { setKpiLayout("grid"); localStorage.setItem("kpiLayout","grid"); }}>
  ...
  />
  <StatCard
    title="Total Prescriptions with Deviation"
  ...
  />
  <StatCard
    title="Total Revenue from Matching Scripts"
    value={totalRevenue}
    decimals={2}
    prefix="$"
    icon="ti-chart-bar"
                    ...
                    <FilterTile label="Insurance / Rx Group">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setSelectedInsurance(val);
                          localStorage.setItem("selectedRx", val || "");
                        }}
                      >
                        <option value="">All</option>
                      ...
                      <thead className="table-light">
                        <tr>
                          {[
                            ...
                            { label: "Diff", key: "difference", align: "end" },                 // derived
                            { label: "Diff / Item", key: "differencePerItem", align: "end" },   // derived
