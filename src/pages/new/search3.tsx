...
const Search3: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try { await loadConfig(); } catch { /* maybe already loaded */ } finally { setReady(true); }
    })();
  }, []);
  ...
  // Details (GET /drug/GetDetails via axiosInstance)
  ...
  const fetchDrugsPage = useCallback(
    async (page: number, append = false) => {
      if (!ready) return;
      if (!drugSearchQuery.trim()) { setDrugs([]); setCurrentPage(1); return; }
  ...
  const debouncedDrugSearch = useCallback(
    debounce(() => {
      if (!drugSearchQuery.trim()) { setDrugs([]); setCurrentPage(1); return; }
      fetchDrugsPage(1, false); setShowDrugSuggestions(true);
    }, 300),
    [fetchDrugsPage, drugSearchQuery]
  );

  useEffect(() => { if (ready) debouncedDrugSearch(); }, [ready, drugSearchQuery, limitSearch, selectedRxGroup, debouncedDrugSearch]);

  /** Infinite scroll in drug dropdown */
  useEffect(() => {
    const el = drugDropdownRef.current; if (!el) return;
  ...
  const onSelectRxGroup = (rg: RxGroupModel) => {
    ...
    setShowRxGroupSuggestions(false);
    // reset downstream
    setDrugSearchQuery(""); setDrugs([]); setCurrentPage(1);
    setSelectedDrug(null); setNdcList([]); setSelectedNdc(""); setNdcSearchQuery(""); setNetDetails(null);
  };

  const onSelectDrug = (drug: DrugModel) => {
    setSelectedDrug(drug); setDrugSearchQuery(drug.name); setShowDrugSuggestions(false);
    const ndcs = Array.from(new Set(drugs.filter(d => d.name === drug.name).map(d => d.ndc)));
    setNdcList(ndcs); setSelectedNdc(ndcs[0] ?? ""); setNdcSearchQuery(ndcs[0] ?? "");
  };

  const onSelectNdc = (ndc: string) => { setSelectedNdc(ndc); setNdcSearchQuery(ndc); setShowNdcSuggestions(false); };
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setApiError(null);
        const { data } = await axiosInstance.get<Prescription>(`/drug/GetDetails?ndc=${encodeURIComponent(selectedNdc)}&insuranceId=${selectedRxGroup.id}`);
        setNetDetails(data ?? null);
      } catch (e: any) {
        if (e?.response?.status === 401 || e?.response?.status === 403) { setNetDetails(null); setApiError("Net price is restricted by the API (401/403)." ); }
  ...
  /** Clear all */
  const clearAll = () => {
    setSelectedRxGroup(null); setRxGroupSearchQuery("");
    setDrugSearchQuery(""); setDrugs([]); setShowDrugSuggestions(false); setCurrentPage(1);
    setSelectedDrug(null); setNdcList([]); setSelectedNdc(""); setNdcSearchQuery("");
    setNetDetails(null); setApiError(null);
  };
  ...
  return (
    <div className="container mid min-vh-100 d-flex flex-column justify-content-center py-4">
      <PageMeta title="Rx Group → Drug → NDC (Authenticated API)" description="Search by Rx Group, then drug and NDC — using the authenticated axiosInstance." />
        ...
        <div className="col-12 col-lg-10 col-xl-8">
          <div className="card shadow-lg border-0 rounded-4 overflow-hidden">
            <div className="card-header text-center py-4 px-5">
              <h4 className="mb-1 fw-semibold"><i className="ti ti-users me-2"></i> RxGroup, Drugs & NDC (Authenticated)</h4>
              <p className="mb-0 text-muted">All reads use the authenticated Axios client. {apiError && <span className="ms-2 text-warning">{apiError}</span>}</p>
            </div>

            <div className="card-body p-5">
              {/* Toggle */}
              <div className="d-flex align-items-center justify-content-between bg-light rounded-3 p-3 mb-4">
                <label htmlFor="limitSearch" className="form-label m-0 fw-medium"><i className="ti ti-filter me-1"></i> Limit search to selected Rx Group</label>
              ...
              {/* Layout: Row 1 full, Row 2 two-cols, Row 3 two-cols */}
              {/* Row 1: Rx Group (full width) */}
              <div className="row g-4">
                <div className="col-12">
                  <div className="position-relative js-suggest">
                    <label className="form-label fw-medium text-dark mb-2"><i className="ti ti-id-badge me-1"></i> Search for Rx Group</label>
                    <div className="input-group input-group-lg">
                      <span className="input-group-text bg-light border-end-0"><i className="ti ti-search text-primary" /></span>
                      <input type="text" className="form-control border-start-0 ps-2" placeholder="e.g., Medi-Cal…" value={rxGroupSearchQuery} onChange={(e) => setRxGroupSearchQuery(e.target.value)} onFocus={() => { hideAllSuggestions(); setShowRxGroupSuggestions(true); }} style={{ height: "52px" }} />
                      {(rxGroupSearchQuery || selectedRxGroup) && (
                        <button className="btn btn-outline-secondary d-flex align-items-center" onClick={clearAll} style={{ height: "52px" }} aria-label="Clear all"><i className="ti ti-x" /></button>
                      )}
                    </div>
                    {showRxGroupSuggestions && filteredRxGroups.length > 0 && (
                      <ul role="listbox" className="position-absolute top-100 start-0 w-100 mt-1 bg-white border rounded-3 shadow-lg" style={{ maxHeight: 240, overflowY: "auto", zIndex: 3000 }}>
                        {filteredRxGroups.map(rg => (
                          <li key={rg.id} role="option" tabIndex={0} onClick={() => onSelectRxGroup(rg)} onKeyDown={(e) => e.key === "Enter" && onSelectRxGroup(rg)} className="list-group-item list-group-item-action border-0 py-3 px-4">{rg.rxGroup}</li>
                        ))}
                      </ul>
                    )}
                  </div>
              ...
              {/* Row 2: Drug | NDC (two columns) */}
              <div className="row g-4 mt-1">
                <div className="col-12 col-lg-6">
                  {!!selectedRxGroup && (
                    <div className="position-relative js-suggest">
                      <label className="form-label fw-medium text-dark mb-2"><i className="ti ti-pill me-1"></i> Search for Drug</label>
                      <div className="input-group input-group-lg">
                        <span className="input-group-text bg-light border-end-0"><i className="ti ti-search text-primary" /></span>
                        <input type="text" className="form-control border-start-0 ps-2" placeholder="e.g., Metformin" value={drugSearchQuery} onChange={(e) => setDrugSearchQuery(e.target.value)} onFocus={() => { hideAllSuggestions(); setShowDrugSuggestions(true); }} style={{ height: "52px" }} />
                      </div>
                      {showDrugSuggestions && uniqueDrugNames.length > 0 && (
                        <ul ref={drugDropdownRef} role="listbox" className="position-absolute top-100 start-0 w-100 mt-1 bg-white border rounded-3 shadow-lg" style={{ maxHeight: 260, overflowY: "auto", zIndex: 3000 }}>
                          {uniqueDrugNames.map(d => (
                            <li key={d.id} role="option" tabIndex={0} onClick={() => onSelectDrug(d)} onKeyDown={(e) => e.key === "Enter" && onSelectDrug(d)} className="list-group-item list-group-item-action border-0 py-3 px-4"><div className="d-flex align-items-center"><i className="ti ti-pill text-primary me-2"></i>{d.name}</div></li>
                          ))}
                          {(isLoadingDrugs || uniqueDrugNames.length >= PAGE_SIZE * currentPage) && (
                            <li className="text-center small text-muted py-3 border-top"><i className="ti ti-loader me-1"></i> Loading more…</li>
                          )}
                        </ul>
                      )}
                    </div>
                ...
                <div className="col-12 col-lg-6">
                  {ndcList.length > 0 && (
                    <div className="position-relative js-suggest">
                      <label className="form-label fw-medium text-dark mb-2"><i className="ti ti-barcode me-1"></i> Search for NDC</label>
                      <div className="input-group input-group-lg">
                        <span className="input-group-text bg-light border-end-0"><i className="ti ti-search text-primary" /></span>
                        <input type="text" className="form-control border-start-0 ps-2" placeholder="Type to filter NDCs…" value={ndcSearchQuery} onChange={(e) => setNdcSearchQuery(e.target.value)} onFocus={() => { hideAllSuggestions(); setShowNdcSuggestions(true); }} style={{ height: "52px" }} />
                      </div>
                      {showNdcSuggestions && filteredNdcList.length > 0 && (
                        <ul role="listbox" className="position-absolute top-100 start-0 w-100 mt-1 bg-white border rounded-3 shadow-lg" style={{ maxHeight: 240, overflowY: "auto", zIndex: 3000 }}>
                          {filteredNdcList.map((ndc, i) => (
                            <li key={`${ndc}-${i}`} role="option" tabIndex={0} onClick={() => onSelectNdc(ndc)} onKeyDown={(e) => e.key === "Enter" && onSelectNdc(ndc)} className="list-group-item list-group-item-action border-0 py-3 px-4"><div className="d-flex align-items-center"><i className="ti ti-barcode text-primary me-2"></i>{ndc}</div></li>
                          ))}
                        </ul>
                      )}
                    </div>
              ...
              {/* Row 3: Net preview (one column) — Keep because it exists in original */}
              {selectedDrug && selectedNdc && (
                <div className="row g-4 mt-1">
                  <div className="col-12">
                    <div className="alert alert-primary d-flex align-items-center gap-3 p-3 rounded-3 mb-0">
                      <div className="bg-white p-3 rounded-3"><i className="ti ti-currency-dollar text-primary fs-4" /></div>
                      <div>
                        <div className="fw-semibold">Net Price</div>
                        <div className="fs-5 fw-bold">{netDetails?.net != null ? `$${netDetails.net}` : (apiError ? "—" : "…")}</div>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {/* Action */}
              <div className="mt-4">
                <button type="button" className="btn btn-primary btn-lg w-100 py-3 fw-semibold" onClick={() => {
                  if (!selectedDrug || !selectedNdc || !selectedRxGroup) return;
                  localStorage.setItem("selectedRx", selectedRxGroup.rxGroup);
                  window.location.href = `/drug/${selectedDrug.id}?ndc=${encodeURIComponent(selectedNdc)}&insuranceId=${selectedRxGroup.id}`;
                }} disabled={!selectedDrug || !selectedNdc || !selectedRxGroup} title={!selectedDrug || !selectedNdc || !selectedRxGroup ? "Pick Rx Group → Drug → NDC" : ""}>
      ...
      {/* Local styles to align with the other pages */}
      <style>{`
